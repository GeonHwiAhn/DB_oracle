...
--급여등급이 S5인 사원의 이름과 급여 조회 SAL_LEVEL
SELECT EMP_NAME, SALARY FROM EMPLOYEE WHERE SAL_LEVEL = 'S5';

--사원번호가 210인 사원의 모든 정보 조회 EMP_ID
...
--급여가 300만원 에서 400만원 사이인 사원의 이름과 급여 조회
...
--EMPLOYEE 테이블에서 부서코드가 'D9'가 아닌 사람들의, 이름 부서코드 전화번호 조회 
SELECT EMP_NAME, DEPT_CODE, PHONE FROM EMPLOYEE WHERE DEPT_CODE != 'D9';

--EMPLOYEE 테이블에서 부서코드가 'D6'인 사원 중에서 급여가 300만원을 초과하는 사람의, 이름 부서코드 급여 조회
...
___#_% __돈%
_를 돈 처럼 사용하길 원했기 때문에 중간 #을 넣어준 것
*/
...
-- 1. EMP_NAME 을 "이름", SALARY를 "급여"로 표시하여 선택
SELECT EMP_NAME AS"이름", SALARY AS "급여" FROM EMPLOYEE;

-- 2. DEPT_CODE 를 "부서코드", 로 JOB_CODE를 "직무코드"로 표시하여 선택
...
-- 4. EMP_ID를 "사원ID", HIRE_DATE을 "입사일"로 표시하여 선택
...
--ENT_YN 가 N이거나 HIRE_DATE  가 2000년 이후인, 사원의 이름과 입사일 선택
SELECT EMP_NAME, HIRE_DATE FROM EMPLOYEE WHERE ENT_YN = 'N' AND HIRE_DATE > TO_DATE ('2000-01-01', 'YYYY-MM-DD') ;
