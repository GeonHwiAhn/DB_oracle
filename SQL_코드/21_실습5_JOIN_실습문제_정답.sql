...
--연봉에 보너스포인트를 적용하시오.
SELECT E.EMP_NAME, J.JOB_NAME, E.SALARY, E.SALARY *(1+ NVL(E.BONUS, '0')) * 12 AS "연봉"
FROM EMPLOYEE E
JOIN SAL_GRADE S ON E.SAL_LEVEL = S.SAL_LEVEL --SAL_GRADE 테이블과 EMPLOYEE 테이블 조인하기 위해 서로 값이 같은 컬럼 연결
JOIN JOB J ON E.JOB_CODE = J.JOB_CODE --JOB 테이블과 EMPLOYEE테이블 조인하기 위해 서로 값이 같은 컬럼 연결
WHERE E.SALARY > s.min_sal; -- 직원의 급여가 최소 급여보다 큰 직원들만 선택할 수 있도록 조건 설정
