...
-- 춘 기술대학교 총장의 이름을 알아낼 수 있은 SQL 문장 작성
...
--10.
--춘 대학교 교수 이름 중 이름이 세 글자가 아닌 교수가 두명 있음
-- 그 교수의 이름과 주민번호 조회
SELECT PROFESSOR_NAME, PROFESSOR_SSN
FROM TB_PROFESSOR
WHERE PROFESSOR_NAME NOT LIKE '___'; --언더바 = 글자수 


--14.
--춘 기술대학교의 2000년도 이후 입학자들은 학번이 A로 시작하게되었음
----2000년도 이전 학번을 받은 학생들의 학번과 이름 조회
--STUDENT_NO. STUDENT_NAME
--FROM TB_STUDENT
SELECT STUDENT_NO, STUDENT_NAME
FROM TB_STUDENT
WHERE SUBSTR(STUDENT_NO, 1, 1) != 'A';      --WHERE STUDENT_NO IS NOT 'A%';
...
SELECT SUBSTR(TERM_NO,1,4) AS "년도" , ROUND(AVG(POINT),1) AS "년도 별 평좀"
FROM TB_GRADE
...
--20
---춘 대학교에 다니는 동명이인 학생들의 이름, 동명이인 수 조회
--FROM TB_STUDENT
