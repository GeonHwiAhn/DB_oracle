...
--2. 최대 급여를 받는 직원이 이름과 급여 조회하기
SELECT MAX(SALARY)
FROM EMPLOYEE e
WHERE EMP_NAME = e.EMP_NAME;
...
WHERE SALARY (서브쿼리 ~~~~~)     --> 급여가 서브쿼리에서 반환된 최소 급여가 같은 행들만 가져오는 것
*/
