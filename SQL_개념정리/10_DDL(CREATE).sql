...
--이름        : USER_NAME / 자료형 VARCHAR2(15) --한글 5자까지 OK
...
-- 삽입 / 수정시 NULL값을 허용하지 않도록 컬럼 레벨에서 제한
...
-- 2. UNIQUE 제약 조건
-- 컬럼에 입력 값에 대해서 종목을 제한하는 제약조건
-- 컬럼 레벨에서 설정 가능, 테이블 레벨에서 설정 가능
-- 단, UNIQUE 제약 조건이 설정된 컬럼에 NULL값 삽입 가능
...
-- CAFE_NAME VARCHAR2(100) NULL값 못들어가게 설정
-- CAFE_PHONE VARCHAR2(20) 핸드폰번호 카페 중복X (방법1)
...
object KH_CAFE 테이블 (TABLE = OBJECT) 존재하기 때문에 중복 생성 X
*/

INSERT INTO KH_CAFE (CAFE_ID, CAFE_NAME, CAFE_PHONE, CAFE_ADDRESS)
        VALUES (1, '맥아커피', '02-1111-1111', '서울시 강남구 광남동 2길');
/*
SQL 오류: ORA-12899: value too large for column "KH_T"."KH_CAFE"."CAFE_PHONE" (actual: 39, maximum: 20)
-> 최대로 넣을 수 있는 공간이 20칸인데 39칸을 넣으려 해서 발생하는 에러

ALTER를 사용해서 컬럼 크기를 수정할 수 있음
ALTER 테이블명 MODIFY (수정할컬럼명 수정할크기);
*/
ALTER TABLE KH_CAFE MODIFY (CAFE_PHONE VARCHAR2(20));
--> 정상적으로 변경할 경우 Table KH_CAFE이(가) 변경되었습니다. 출력
...
-- (방법 3)
    컬럼명 자료형(정수자리값,실수자리값) CONSTRAINT 인덱스에저장할이름명 PRIMARY KEY(연결할컬럼명)
);
*/
...
-- UNIQUE는 컬럼에서 NULL이 한개는 허용됨, 두개이상 안됨
...
CONSTRAINT 인덱스값 REFERENCE 컬럼명(참조할키);
*/
...
INSERT INTO USER_GRADE VALUES (00, '임시회원'); -->정수에서 맨 앞에 0은 사라짐
--> 핸드폰번호를 기입할 때는 VARCHAR2를 사용해서 010이 모두 들어갈 수 있도록 설정
...
--배달어플에서 A카페 폐업
---배달어플 -> A카페 마음대로 폐업은 못하지
----A카페가 열어놓은 메뉴들은 모두 삭제하고 가

-- CASCADE --부모키 삭제 시 참조하는 자식의 행도 모두 삭제
...
/*
ORA-02290: check constraint (KH_T.SYS_C007226) violated
...
INSERT INTO USER_CHECK
VALUES (3, 'user03', 'pw03', '강영희', '여');


-- CHECK 사용방법
-- 1번 방법 따로 INDEX에 기록하지 않고 조건설정만 할 경우
컬럼명 자료형  CHECK(컬럼명 IN ('조건1', '조건2'))

--2번 방법 따로 INDEX에 기록한 다음 조건 설정한 경우 (한줄 작성)
컬럼명 자료형 CONSTRAINT 인덱스에기록할이름 CHECK(컬럼명 IN ('조건1', '조건2'))

--3번 방법 따로 INDEX에 기록한 다음 조건 설정한 경우 (여러줄 작성)
컬럼명 자료형,
