...
포괄 조인 --> 왼쪽 외부 조인(LEFT JOIN), 오른쪽 외부 조인(RIGHT OUTER), 전체 외부 조인(FULL OUTER)
자체 조인 --> JOIN ON
교차 조인 --> CROSS JOIN
...
FROM EMPLOYEE;
-- 직원번호, 직원명, 부서코드는 EMPLOYEE테이블에서 조회 가능
/*
...
--ANSI JOIN ON LOCATION_ID = LOCAL_CODE
SELECT DEPT_TITLE AS "부서명", LOCAT_CODE AS "코드명", LOCAL_NAME AS "지역명"
FROM DEPARTMENT
...
SELECT e.EMP_ID, e.EMP_NAME, e.JOB_CODE, j.JOB_CODE
...
/*****
NATURAL JOIN (자연조인) 
동일한 타입과 이름을 가진 칼럼이 있는 테이블 간의 조인을 간단히 표현한 방법
...
-- 1) LEFT [OUTER] JOIN
--합치기에 사용한 두 테이블 중에서 왼쪽편에 작성된 테이블의
--컬럼수를 기준으로 JOIN
...
--왼쪽에 있는 컬럼의 수에 맞게 표시를 넣어라 (NULL값이라도 넣어라)




-- 2) RIGHT [OUTER] JOIN
--합치기에 사용한 두 테이블 중에서 오른쪽편에 작성된 테이블의
--컬럼수를 기준으로 JOIN
...
WHERE DEPT_CODE(+) = DEPT_ID;
--DEPT_CODE와 DEPT_ID값이 일치하지 않아도
--오른쪽에 있는 컬럼의 수에 맞게 표시를 넣어라 (NULL값이라도 넣어라)
