...
--SELECT문에 살술 연산을 직접 작성하면
--조회결과(RESULT SET)에 연산 결과가 반영되어 조회됨
*/

--EMPLOYEE 테이블에서
--모든 사원의 이름, 급여, 급여 + 100만원 조회
SELECT EMP_NAME, SALARY + 1000000 FROM EMPLOYEE;
...
컬럼 값이 존재하지 않는 경우 IS NULL;
컬럼 값이 존재하는 경우       IS NOT NULL;
*/
...
/*
LIKE
비교하려는 값이 특정한 패턴을 만족시키면 조회하는연산자
WHERE 컬럼명 LIKE '패턴'

%(포함)
- %A : 문자열이 앞은 어떤 문자든 포함되고 마지막은 A
  예 : %륨 검색창에 륨으로끝나는 단어를 검색한 것과 비슷
...
- A_ : A뒤에 아무거나 한 글자만 있는 문자열
  예 : AB, AI, AQ, A가
    가_ : 가로 시작하는 두글자 단어만 가능
    나___ : 나로 시작하는 세글자 단어만 검색
...
SELECT EMP_NAME AS "이름", DEPT_CODE AS "부서코드", JOB_CODE AS "직급코드" FROM EMPLOYEE 
...
-- 주의할 점 : ORDER BY에서는 별칭 사용 가능하지만
-- WHERE 절의 경우 조건이기 때문에 ***별칭 사용 불가능***
...
J6에서는 2명 이상이 존재하기 때문에 J6직급의 직원들의 이름이 오름차순 정렬
