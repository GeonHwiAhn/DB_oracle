...
-- ALTER(바꾸다, 수정하다)
-- 수정가능한 것 컬럼(추가/수정/삭제) 제약조건(주가/삭제) 이름변경(테이블, 컬럼, 제약조건)
...
---> 데이터 타입 VARCHAR2(100) -> VARCHAR2(10)   데이터 타입이 10 초과되는건 불가

-- 컬럼 삭제 : ALTER TABLE 테이블명 DROP (삭제할컬럼명);
-->              ALTER TABLE 테이블명 DROP COLUMN 삭제할컬럼명;

--> 테이블에는 최소 1개 이상의 컬럼이 존재해야하기 때문에 모든 컬럼 삭제X


--(추가)
-- DEPT_COPY 테이블에 CNAME VARCHAR(20) 컬럼 추가
...
--(수정)
-- DEPT_COPY 테이블의 DEPT_ID 컬럼의 데이터 타입을 CHAR(2) -> VARCHAR(3) 변경
...
CREATE TABLE TB2(
    ...
    TB2_COL NUMBER REFERENCES TB1 -- TB1테이블의 PK 값을 참조
);
...
--> 제약조건까지 모두 삭제 조건을 없애버려서 부모자식 끊음
