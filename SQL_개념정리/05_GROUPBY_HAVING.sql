...
여러개의 결과 값을 산출하기 위해 그룹 함수가 적용된 그룹의 기준을 ORDER BY절에 기술해서 사용
...
-- EMPLOYEE 테이블에서
-- 부서코드와 부서별 보너스를 받는 사원의 수
--1. SELECT 절에서 부서코드랑 보너스사원 수 카운트 진행
...
--HAVING절 : 그룹 함수로 구해올 그룹에 대한 조건을 설정할 때 사용
--            (그룹에 대한 조건, 어떤 그룹만 조회하겠다)
            
--부서별 평균 급여가 3000000원 이상인 사원만 조회하고 오름차순 정렬
-->급여가 3000000 이상인 직원들만 대상으로 각 부서의 평균 급여 계산
--->그룹화 전에 필터링
...
D1	3010700
D3	5092500
D6	3423333.33333333333333333333333333333333
D9	8000000
...
D1	    종합	7820000 <-- D1 부서 전체의 급여 합계 (소계)
D2	    J4	    6520000 <-- D2 부서의 J4 직급 급여 합계
D2	    종합	6520000 <-- D2 부서의 전체 급여 합계 (소계)
D5	    J3	    3500000 <-- D5 부서의 J3 전체 급여 합계
D5	    J5	    8460000 <-- D5 부서의 J5 전체 급여 합계
D5	    J7	    3800000 <-- D5 부서의 J7 전체 급여 합계
D5	    종합	15760000 <-- D5 부서의 전체(J3, J5, J7) 급여 합계 (소계)
D6	    J3	    7300000 <-- D6 부서의 J3 전체 급여 합계
D6	    J4	    2800000 <-- D6 부서의 J4 전체 급여 합계
D6	     종합	10100000 <-- D6 부서의 (J3, J4) 급여 합계 (소계)
D8	    J6	    6986240 <-- D8 부서의 J6 전체 급여 합계
D8  	 종합  6986240 <-- D6 부서의 전체(J6) 급여 합계 (소계)
D9	    J1	    8000000 <-- D9 부서의 J 전체 급여 합계
D9	    J2	    9700000 <-- D9 부서의 J2 전체 급여 합계
D9	    종합	17700000 <-- D9 부서의 전체(J1, J2) 급여 합계 (소계)
null    	J6	2320000 <-- J6 전체 급여 합계
null	    J7	2890000 <-- J7 전체 급여 합계
null    null	5210000
...
CUBE는 가능한 모든 것을 조합하고 조합에 해당 결과와 총합 계산
부서 직급 SUM(SALARY)
...
--주의할점 : 집합 연산에 사용되는 SELECT문은 SELECT절이 동일해야 함

--UNION은            OR같은 개념 --> 중복 제거
--INTERSECT          AND 같은 개념
--MINUS는            차집합 같은 개념
--UNION ALL         OR 결과값에 AND 결과 같이 더해진 값 --> 중복이 제거되지 않은 채로 합쳐짐
...
-- 1)부서코드가 'D5' 또는 'D6' 인 사원 이름, 부서코드 조회
...
--부서코드가 D5 중 급여가 300만원 초과인 직원 제외
